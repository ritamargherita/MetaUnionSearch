...
def predict_unionability_on_threshold(input_file, output_file, threshold):
    ...
    df_input['predicted_unionable'] = df_input['score'].apply(lambda x: 1 if x>= threshold else 0) #remember to change the threshold
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python semanticSimilarityEvaluation.py <path_input_file> <path_output_file> <threshold_for_unionability>")
        sys.exit(1)
